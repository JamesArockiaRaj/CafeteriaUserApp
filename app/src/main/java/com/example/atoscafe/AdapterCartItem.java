    ...
    public void onBindViewHolder(@NonNull HolderCartItem holder, int position) {
        ...
        holder.itemRemoveTv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                double tx = Double.parseDouble((((ShopDetailsActivity)context).TotalPriceTv.getText().toString().trim().replace("₹","")));
                double totalPrice = tx - Double.parseDouble(cost.replace("₹",""));
                double gst = Double.parseDouble((((ShopDetailsActivity)context).gst.replace("₹","")));
                double sTotalPrice = Double.parseDouble(String.format("%.2f",totalPrice))-Double.parseDouble(String.format("%.2f",gst));
                ((ShopDetailsActivity)context).TotalPrice=0.00;
                ((ShopDetailsActivity)context).sTotalTv.setText("₹"+String.format("%.2f",sTotalPrice));
                ((ShopDetailsActivity)context).TotalPriceTv.setText("₹"+String.format("%.2f",Double.parseDouble(String.format("%.2f",totalPrice))));
